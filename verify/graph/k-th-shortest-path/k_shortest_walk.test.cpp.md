...
data:
  ...
  bundledCode: "#line 1 \"verify/graph/k-th-shortest-path/k_shortest_walk.test.cpp\"\
    ...
    \ kth_shortest_path(vector<vector<pair<int,ll>>> &adj,int s,int t,int k){\n  \
    \  int n=adj.size();\n    vector<vector<pair<int,ll>>> radj(n);\n    for(int u=0;u<n;u++)for(auto\
    \ [v,w]:adj[u])radj[v].emplace_back(u,w);\n    using P = pair<ll,int>;\n    priority_queue<P,vector<P>,greater<P>>\
  ...
  dependsOn:
  - src/contest/template.hpp
  - src/graph/k-th-shortest-path.hpp
  ...
  verifiedWith: []
documentation_of: verify/graph/k-th-shortest-path/k_shortest_walk.test.cpp
layout: document
redirect_from:
- /verify/verify/graph/k-th-shortest-path/k_shortest_walk.test.cpp
- /verify/verify/graph/k-th-shortest-path/k_shortest_walk.test.cpp.html
